...
void leer_csv(char *filename, int *numeros, int n_args) {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        // if (nproducto != 40) continue;  // Filtrar por NPRODUCTO

        // Verificar si el primer número aparece en las columnas R1-R6
        for (j = 0; j < 6; j++) {
            if (numeros[0] == R[j]) {
                frecuencia[0]++;
                filas_validas[counter++] = fila_actual;  // Guardar fila válida
                break;
            }
        }
    ...
    for (i = 1; i < n_args; i++) {
        ...
        while (fgets(line, sizeof(line), file)) {
            sscanf(line, "%d,%d,%d,%d,%d,%d,%d,%f,%s", &nproducto, &concurso, &R[0], &R[1], &R[2], &R[3], &R[4], &bolsa, fecha);
